...
# Ensure multi-line TEXT never causes an empty-command exec
: "${TEXT:=}"
...
# Expose run/url and host with safe expansions
...
cat > "${STATUS_FILE:-}" <<EOF
{
  ...
  "runner_hostname": "${HOST:-}",
  "workflow_url": "${RUN_URL:-}"
}
EOF

git -C "${REPO:-}" add "${HANDOVER_FILE}" "${STATUS_FILE}"
